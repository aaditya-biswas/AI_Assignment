"""
engine_final.py: A complete, architecturally sound rewrite of the game engine.
This version adds position history tracking to help the AI avoid repetitive loops.
"""
from config import *
...
class GameEngine:
    def __init__(self):
        ...
        self.move_log = []
        # --- NEW: Keep a history of board states to detect repetitions ---
    ...
    def get_initial_board(self):
        ...
        board[0] = [BLACK_KNIGHT, BLACK_BISHOP, BLACK_ROOK, BLACK_KING, BLACK_ROOK, BLACK_BISHOP]
        board[1] = [BLACK_PAWN] * BOARD_WIDTH
        board[6] = [WHITE_PAWN] * BOARD_WIDTH
        board[7] = [WHITE_KNIGHT, WHITE_BISHOP, WHITE_ROOK, WHITE_KING, WHITE_ROOK, WHITE_BISHOP]
        return board
    ...
    def make_move(self, move):
        ...
        # --- NEW: Update history after every move ---
        self.update_position_history()

    def undo_move(self):
        if not self.move_log: return
        # --- NEW: First, decrement the history count for the current position ---
        board_tuple = tuple(tuple(row) for row in self.board)
        key = (board_tuple, self.white_to_move)
    ...
    def get_legal_moves(self):
        """(Generate-then-Validate) Get all legal moves for the current player."""
        possible_moves = self._get_all_possible_moves()
        legal_moves = []
        for move in possible_moves:
            self.make_move(move)
            # The king of the player who JUST moved should not be in check
    ...
    def _is_square_attacked(self, square, friendly_color):
        return (self._is_attacked_by_pawn(square, friendly_color) or
                ...
                self._is_attacked_by_rook(square, friendly_color) or
    ...
    def _is_attacked_by_rook(self, square, friendly_color):
        r, c = square
        opponent_rook = BLACK_ROOK if friendly_color == 'w' else WHITE_ROOK
        for dr, dc in [(-1,0),(1,0),(0,-1),(0,1)]:
            for i in range(1, 8):
                ...
                piece = self.board[end_r][end_c]
                if piece != EMPTY_SQUARE:
                    if piece == opponent_rook: return True
                    break
        return False
        ...
        for r, row in enumerate(self.board):
            for c, piece in enumerate(row):
                if piece.startswith(turn_color):
                    ...
                    elif ptype == 'R': self._get_rook_moves(r, c, moves)
