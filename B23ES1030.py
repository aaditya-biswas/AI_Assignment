...
Roll Number: B23ES1030
Features: Iterative Deepening, Delta-Pruned Quiescence Search, MVV-LVA Move Ordering, 
King Safety, and Exception-Safe Board State Management.
"""
import time
from config import (
...
class TimeoutException(Exception):
    """Custom exception raised when the per-turn search deadline passes."""
    pass

class B23ES1030:
    def __init__(self, engine, depth=12):
        self.engine = engine
        self.depth = depth          # Required by game_runner.py loggers
        self.max_depth = depth
        self.nodes_expanded = 0
        self.time_used = 0.0

    def get_best_move(self):
        """Returns the optimal Move object within the allocated time budget."""
        t0 = time.monotonic()
        self.nodes_expanded = 0
        ...
        # Time budget per turn (~75 turns max)
        ...
        for current_depth in range(1, self.max_depth + 1):
            ...
            # Order moves using indices to avoid object mutation issues
            ...
            try:
                ...
                for orig_idx, move in ordered_moves:
                    ...
                    try:
                        ...
                        # GUARANTEED UNDO: Keeps the actual board state pristine
        ...
        # Select final move from a fresh legal moves query
        final_moves = self.engine.get_legal_moves()
        if not final_moves:
            return None
        if best_move_idx < len(final_moves):
            return final_moves[best_move_idx]
        return final_moves[0]

    def alphabeta(self, depth, alpha, beta, piece_count, deadline):
        """Alpha-Beta Negamax pruning with checkmate depth penalty."""
    ...
    def quiescence(self, alpha, beta, piece_count, deadline, qdepth=0):
        """Tactical capture search with Delta Pruning."""
        self.nodes_expanded += 1
        if (self.nodes_expanded & 1023) == 0:
            if time.monotonic() >= deadline:
                raise TimeoutException()
    ...
    def evaluate_board(self, piece_count):
        """Calculates material score, PST positions, and king positioning."""
    ...
    def _mate_drive_bonus(self, winner_king, loser_king, h, w):
        """Encourages cornering the enemy king in late endgames."""
        lr, lc = loser_king
        wr, wc = winner_king
        score = 0
